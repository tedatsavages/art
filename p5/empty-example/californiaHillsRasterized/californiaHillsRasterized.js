...
function draw() {
  ...
  for (let u = 0; u < 8; u++){
    ...
    for(let i = 0; i < numPiece; i ++){
        ...
        let rectXDim = 64*((1-sin((frameCount + ((i+u)*475)*((freq.value()/255))*freqFX)*(tMult/255)+(90/(u+1)))*abs(noise((frameCount/180)+((i+(u*64))*noiseFreq.value()))))/2);
        let rectYDim = 64*((1-cos((frameCount + ((i+u)*475)*((freq.value()/255))*freqFY)*(tMult/255)+(90/(u+1)))*abs(noise((frameCount/180)+((i+(u*64))*noiseFreq.value()))))/2);
        stroke(60);
        fill(195, 118, 222, 55);
